...
class Admin(commands.Cog, name="Admin Commands"):
    ...
    async def cog_command_error(self, ctx: commands.Context, error: Exception) -> None:
        if isinstance(error, commands.MissingPermissions):
            ...
            if not command:
                return
            logging.info(f"[{ctx.author.id}] failed command [{command.name}] // noPerm [{error}]")
    ...
    async def sendVerEmbed(self, ctx:commands.Context):
        logging.info("running command sendVerEmbed")
    ...
    async def reload(self, ctx:commands.Context):
        logging.info("running command reload")
    ...
    async def echo(self, ctx:commands.Context, *, repeat:str):
        logging.info("running command echo")
    ...
    async def shutdown(self, ctx:commands.Context):
        logging.info("running command shutdown")
    ...
    async def giveLog(self, ctx:commands.Context):
        logging.info("running command giveLog")
